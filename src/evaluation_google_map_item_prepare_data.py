...
class GenerateGoogleMapItemInput:
    def __init__(self):
        self.base_dir = os.path.join('..', 'intermediate', 'google_map', 'evaluation', 'item')
        self.original_dir = os.path.join(self.base_dir, 'original')
        self.input_dir = os.path.join(self.base_dir, 'input')
        os.makedirs(self.input_dir, exist_ok=True)

        self.pos_json = os.path.join(self.original_dir, 'pos_FOP_text.json')
        self.neg_json = os.path.join(self.original_dir, 'neg_FOP_text.json')
    ...
    def generate_all(self):
        ...
        vocab_pos, pos_idx_lists, pos_num = self._build_vocab_and_indices(pos_dict)
        ...
        vocab_neg, neg_idx_lists, neg_num = self._build_vocab_and_indices(neg_dict)
        neg_tensor_full = self._tensor_from_indices(neg_idx_lists, len(vocab_neg))

        pos_train = pos_tensor_full[:self.train_num]
        pos_test = pos_tensor_full[self.train_num:]
        neg_train = neg_tensor_full[:self.train_num]
        neg_test = neg_tensor_full[self.train_num:]
...
if __name__ == '__main__':
    generator = GenerateGoogleMapItemInput()
