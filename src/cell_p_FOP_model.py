...
class FOPModelTrainer:
    def __init__(self, device='cuda:0'):
        ...
        self.model_dir = os.path.join(self.base_dir, 'intermediate', 'cell_p')
        self.figure_dir = os.path.join(self.base_dir, 'results', 'cell_p', 'wordcloud')
        for d in [self.intermediate_dir, self.model_dir, self.figure_dir]:
        ...
        self.num_epochs = 300
        self.batch_size = 512
        self.pos_latent_dim = 5
        self.neg_latent_dim = 3

    def load_data(self, fname):
        path = os.path.join(self.intermediate_dir, fname)
        return torch.load(path)

    def train_one(self, tensor, input_dim, latent_dim, model_name):
    ...
    def train_models(self):
        pos_tensor = self.load_data('pos_tensor.pt')
        neg_tensor = self.load_data('neg_tensor.pt')
        self.pos_input_dim = pos_tensor.shape[1]
        self.neg_input_dim = neg_tensor.shape[1]

        self.pos_network = self.train_one(pos_tensor, self.pos_input_dim, self.pos_latent_dim, 'pos_model')
        self.neg_network = self.train_one(neg_tensor, self.neg_input_dim, self.neg_latent_dim, 'neg_model')

    def compute_character_vector(self):
        pos_tensor = self.load_data('pos_tensor.pt').to(self.device)
        neg_tensor = self.load_data('neg_tensor.pt').to(self.device)
        ...
        proportions = []
        with torch.no_grad():
            for pos_batch, neg_batch in tqdm(dataloader, desc="Computing proportions"):
                pos_batch = pos_batch.to(self.device)
                neg_batch = neg_batch.to(self.device)
                pos_prop, _ = self.pos_network.evaluate(pos_batch)
                neg_prop, _ = self.neg_network.evaluate(neg_batch)
                prop_batch = torch.cat((pos_prop, neg_prop), dim=1)
                proportions.append(prop_batch.cpu())

        proportion_tensor = torch.cat(proportions, dim=0)
        char_vec = torch.zeros_like(proportion_tensor)
        char_vec[:, :self.pos_latent_dim] = (self.pos_latent_dim / (self.pos_latent_dim + self.neg_latent_dim)) * proportion_tensor[:, :self.pos_latent_dim]
        char_vec[:, self.pos_latent_dim:] = (self.neg_latent_dim / (self.pos_latent_dim + self.neg_latent_dim) / (self.neg_latent_dim - 1)) * (1 - proportion_tensor[:, self.pos_latent_dim:])

        out_path = os.path.join(self.intermediate_dir, 'character_vector.pt')
        torch.save(char_vec, out_path)
        print(f"Character vector saved to {out_path}")

    def visualize_topics(self, polarity, model_name, latent_dim):
        ...
        input_dim = self.pos_input_dim if polarity == 'pos' else self.neg_input_dim
        network = FOPNetwork(input_dim, self.hidden_dim, latent_dim, self.dropout).to(self.device)
        model_path = os.path.join(self.model_dir, f'{model_name}.pt')
    ...
    def run(self):
        self.train_models()
        self.compute_character_vector()
        self.visualize_topics('pos', 'pos_model', self.pos_latent_dim)
        self.visualize_topics('neg', 'neg_model', self.neg_latent_dim)

if __name__ == '__main__':
    trainer = FOPModelTrainer(device='cuda:0')
    trainer.run()