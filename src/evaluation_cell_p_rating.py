...
BASE_DIR = os.path.join('..', 'intermediate', 'cell_p', 'evaluation', 'rating')
...
class PMFRatingNetwork(nn.Module):
    ...
    def norm_loss(self, behavior):
        user_vector = self.user_embedding(behavior[:, 0].to(torch.long))
        item_vector = self.item_embedding(behavior[:, 1].to(torch.long))
        loss = torch.square(user_vector) + torch.square(item_vector)
        return loss.sum()

class ModelEvaluator:
    def __init__(self):
        self.character_vector = torch.load(os.path.join('..', 'intermediate', 'cell_p', 'character_vector.pt'))
    ...
    def _run_complete_model_single(self, verbose=False):
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                user_motivation.append(torch.softmax(mean, dim=-1).cpu())
    ...
    def _run_pmf_single(self, verbose=False):
        ...
        for epoch in range(180):
            ...
            for batch in train_loader:
                inferred = net(batch)
                loss = net.rating_loss(batch, inferred) + 0.25 * net.norm_loss(batch)
                optimizer.zero_grad()
                loss.backward()
    ...
    def _run_utadis_single(self, verbose=False):
        ...
        for i in tqdm(range(num_user), desc="UTADIS users", disable=not verbose):
            str_i = str(i)
    ...
    def _run_nsa_single(self, verbose=False):
        ...
        def compute_metrics(df, desc):
            text_list = df['text'].astype(str).tolist()
            rating_list = df['rating'].tolist()
            mae, mse = 0, 0
            for i in tqdm(range(len(text_list)), desc=desc, disable=not verbose):
                review = text_list[i]
        ...
        train_mae, train_mse, train_cnt = compute_metrics(train_df, "NSA train")
        test_mae, test_mse, test_cnt = compute_metrics(test_df, "NSA test")
