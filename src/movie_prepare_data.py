...
class DataPrepare:
    def __init__(self):
        ...
        self.output_dir = os.path.join(self.base_dir, 'intermediate', 'movie')
        os.makedirs(self.data_dir, exist_ok=True)
        os.makedirs(self.output_dir, exist_ok=True)

    def derive_FOP_tensor(self):
        pos_path = os.path.join(self.data_dir, 'FOP_pos.json')
        ...
        vocab_pos = Vocab(pos_list, min_freq=5)
        pos_idx_list = vocab_pos[pos_list]
        pos_tensor = torch.zeros((len(pos_list), len(vocab_pos)))
        for i in tqdm(range(len(pos_list)), desc='Processing FOP_pos'):
        ...
        neg_path = os.path.join(self.data_dir, 'FOP_neg.json')
        ...
        for i in tqdm(range(len(neg_list)), desc='Processing FOP_neg'):
    ...
    def derive_user_tensor(self):
        ...
        time_one_hot = nn.functional.one_hot(torch.arange(num_times)).unsqueeze(0).repeat(num_users, 1, 1)
