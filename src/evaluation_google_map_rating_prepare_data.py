...
class GenerateGoogleMapInput:
    def __init__(self):
        self.base_dir = os.path.join('..', 'intermediate', 'google_map', 'evaluation', 'rating')
        ...
        self.character_vector_path = os.path.join('../intermediate/google_map', 'character_vector.pt')

        # 硬编码的维度（与原始代码一致）
        self.unique_user = 4936
        self.unique_time_period = 6
        self.unique_item = 2275
    ...
    def generate_complete_model(self):
        # ------------------ 训练集 user_tensor (bag-of-items per user per time) ------------------
    ...
    def generate_model1(self):
        ...
        # ------------------ 训练集 user_tensor (bag-of-items per user) ------------------
    ...
    def generate_utadis(self):
        char_vec = torch.load(self.character_vector_path)  # 假设形状为 (unique_item, feature_dim)
        df_train = pd.read_csv(self.train_csv)
        df_test = pd.read_csv(self.test_csv)
        ...
        for user_id, group in grouped_train:
            ...
            item_vecs = char_vec[torch.tensor(item_list)]  # 使用 item_id 直接索引
...
if __name__ == '__main__':
    generator = GenerateGoogleMapInput()
