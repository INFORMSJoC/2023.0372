...
class FOPModelTrainer:
    def __init__(self, device='cuda:0'):
        ...
        self.figure_dir = os.path.join(self.base_dir, 'results', 'movie', 'wordcloud')
    ...
    def train_one(self, tensor, latent_dim, model_name):
        ...
        model_path = os.path.join(self.intermediate_dir, f'{model_name}.pt')
    ...
    def train_models(self):
        ...
        self.pos_network = self.train_one(pos_tensor, self.pos_latent_dim, 'pos_model')
        self.neg_network = self.train_one(neg_tensor, self.neg_latent_dim, 'neg_model')

    def visualize_topic(self, polarity, latent_dim, json_fname, min_freq):
        json_path = os.path.join(self.data_dir, json_fname)
        if not os.path.exists(json_path):
            print(f"JSON file not found: {json_path}, skipping visualization.")
            return
        with open(json_path, 'r') as f:
            data = json.load(f)
        token_lists = [data[str(i)] for i in range(len(data))]
        vocab = Vocab(token_lists, min_freq=min_freq)

        input_dim = self.pos_input_dim if polarity == 'pos' else self.neg_input_dim
        network = FOPNetwork(input_dim, self.hidden_dim, latent_dim, self.dropout).to(self.device)
        model_path = os.path.join(self.intermediate_dir, f'{polarity}_model.pt')
    ...
    def visualize_pos_topic(self):
        self.visualize_topic('pos', self.pos_latent_dim, 'FOP_pos.json', min_freq=5)

    def visualize_neg_topic(self):
        self.visualize_topic('neg', self.neg_latent_dim, 'FOP_neg.json', min_freq=3)
    ...
    def derive_item_proportion(self):
        ...
        char_vec[:, :self.pos_latent_dim] = (self.pos_latent_dim / (self.pos_latent_dim + self.neg_latent_dim)) * item_proportion[:, :self.pos_latent_dim]
        char_vec[:, self.pos_latent_dim:] = (self.neg_latent_dim / (self.pos_latent_dim + self.neg_latent_dim) / (self.neg_latent_dim - 1)) * (1 - item_proportion[:, self.pos_latent_dim:])
        torch.save(char_vec, os.path.join(self.intermediate_dir, 'character_vector.pt'))
        print('character_vector.pt saved.')
